"""
MalabarDB Phase 1, stage 2: verify that every ingredient line parsed correctly.

Checking 1739 lines by hand is not viable, and spot-checking gives no coverage
guarantee. Instead, two independent extractors parse every line and their
DISAGREEMENT selects the rows a human looks at:
    ...
    keyword regex     dry_fresh           -- sole source, library has no such field

Where the two agree on a field, that field is accepted. Where they disagree,
the row enters the review queue. State disagreement is checked by obsering if 
a line has a " - X" modifier where state info would live but was undetected 

OUTPUTS
    full_corpus_labels.csv    every line, every extracted field, every flag.
                              Machine-generated, regenerable, do not hand-edit.
    review_queue_parsing.csv  only the flagged lines, with blank corrected_*
                              columns. This is the file you edit.

Rerunning is safe: corrections already typed into the queue are carried
forward and only newly flagged rows arrive blank.

Next stage: merge_corrections.py folds the queue back over the full table.
"""
...
# Unit vocabulary derived by scanning THIS corpus for the word following a
# leading quantity, not from a generic English unit list. Adjectives
# ("dry", "green", "small") occur in that position too and were excluded by
# reading the matches.
UNITS = {
...
# Alternation returns the FIRST match, not the longest, so the mixed fraction
# "2-1/2" must be tried before the range "2-3" or it gets sliced as "2-1".
QTY_RE = re.compile(
    r'^\s*('
    r'\d+\s+to\s+\d+'
    r'|\d+\s*-\s*\d+\s*/\s*\d+'
...
# CORRECTION columns (typed in, and the only ones merge_corrections.py reads
# back). ip_comment/ip_purpose exist to explain a state miss -- e.g. the info
# was there but ingredient-parser filed it under comment, not preparation --
# they never feed the pipeline themselves.
QUEUE_COLUMNS = [
...
def run_ip(raw_line):
    ...
    opinion only; state is this method's sole contribution. comment/purpose
    are kept as REVIEW CONTEXT ONLY -- they explain a state miss (info landed
    in one of these instead of preparation) but neither feeds back into the
    pipeline; only corrected_state does."""
...
def name_patch(regex_name, ip_name):
    ...
    Ratalu", with the rest pushed into comment. Only overrides when the
    library's tokens are a strict subset of the regex tokens, i.e. something is
    genuinely missing rather than merely phrased differently."""
...
def get_modifier_text(raw_line):
    ...
    LIMITATION: pre-nominal states ("Cooked rice") carry no dash and are
...
def build():
    ...
    ri['flag_name'] = ri.apply(
        lambda r: _norm(r.regex_name) != _norm(r.ip_patched_name), axis=1)
    ...
    if os.path.exists(paths.REVIEW_QUEUE_PARSING):
        done = queue[CORRECTION_COLUMNS].apply(
            lambda r: r.astype(str).str.strip().ne('').any(), axis=1).sum()
